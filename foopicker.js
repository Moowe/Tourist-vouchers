...
var FooPicker = (function () {
  ...
  function FooPicker() {
	...
	_self.reinit = function(){
		if (arguments[0] && typeof arguments[0] === "object") {
			_self.options = extendOptions(_self.options, arguments[0]);
    	}
	}
      ...
      if (pickerDiv) {
        ...
        if (el && el.length) {
          for (var count = 0; count < el.length; count++) {
			...
			let curDate = new Date(el[count].dataset.year, el[count].dataset.month, el[count].dataset.day, 0, 0, 0, 0).valueOf();  
			let minDate = 0;
			if(instance.options.mindate)
			{
				minDate = new Date(instance.options.mindate.y, instance.options.mindate.m, instance.options.mindate.d, 0, 0, 0, 0).valueOf();  
			}
            if ((instance.options.disable && instance.options.disable.indexOf(currentDate) !== -1) || (instance.options.mindate &&  ( minDate > curDate ) )) {
